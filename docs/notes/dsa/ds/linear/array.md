...
![数组](assets/array/array_definition.png)
...
![数组-插入元素](assets/array/array_insert_element.png)
...
![数组-删除元素](assets/array/array_remove_element.png)
...
顺序表的的遍历通常通过**循环结构**实现。根据不同编程语言的封装，通常有**索引遍历**与**对象迭代遍历（直接遍历）**两种方式，[时间复杂度](../../anal/time.md#线性阶)为 $O(n)$：
