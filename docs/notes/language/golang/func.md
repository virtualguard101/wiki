...
在Golang中，参数传递都是**值传递**，不存在[引用传递](../cpp/cs106l/01-init-reference.md#通过引用传递变量)，即函数中对形参的操作不会改变函数作用域外实参的值。
...
熟悉JS的话应该能发现这类似JavaScript中[回调函数](../javascript/js.md#回调函数)的使用:
...
所谓**闭包**，其实就是**匿名函数**+**捕获的变量**，类似C++中的[lambda表达式](../cpp/cs106l/06-lambda_function.md)与JavaScript中的[箭头函数](../javascript/js.md#箭头函数)。
