...
空闲表法属于**连续分配方式**，与内存的[动态分区分配](3-memory-management.md#动态分区分配)类似，为每个文件分配一块连续的存储空间。
...
文件对象包含与该文件相关联的目录项对象，包含该文件的文件系统、文件指针等，还包含在该文件上的一系列操作函数（[文件操作](4-file-sys-overview.md#文件操作)函数）
...
- Windows维护一个扩展的[两级目录结构](4-dir.md#两级目录)，用驱动器字母表示设备和卷
...
[^4]: [索引节点-文件系统概述](4-file-sys-overview.md#索引节点)