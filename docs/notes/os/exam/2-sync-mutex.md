...
该算法解决了单标志法的“空闲让进”问题，但存在<span style="color: red;">竞态条件</span>，导致两个进程同时进入临界区。

- **违反互斥原则**: 存在竞态条件，可能导致两个进程同时进入临界区。<span style="color: red;">这是一个致命的问题，决定了该算法无法在实际系统中应用</span>

    原因在于*检查与赋值*不是**原子操作**，即两个操作无法一气呵成。

    ```cpp
    // 危险的执行序列
    // 时间点1：进程0执行 flag[0] = true
    // 时间点2：进程1执行 flag[1] = true  
    // 时间点3：进程0检查 flag[1] = true，但此时进程1还没检查flag[0]
    // 时间点4：进程1检查 flag[0] = true，但此时进程0还没检查flag[1]
    // 结果：两个进程都认为对方不想进入，同时进入临界区！❌

    void process0() {
        flag[0] = true;        // 时间点1
        while (flag[1]) {      // 时间点3：检查时flag[1]可能还是false
            // 等待
        }
        critical_section();    // 可能和进程1同时执行！
    }

    void process1() {
        flag[1] = true;        // 时间点2
        while (flag[0]) {      // 时间点4：检查时flag[0]可能还是false
            // 等待
        }
        critical_section();    // 可能和进程0同时执行！
    }
    ```
