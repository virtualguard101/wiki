...
- **基于顺序搜索的分配算法**:
    ...
    - 首次适应算法 (First Fit): 空闲分区从低地址开始按序排列，每次分配时，找到第一个满足要求的空闲区进行分配。

    - 邻近适应算法 (Next Fit): 也叫**循环首次适应算法**，从首次适应算法演变而来，不是从低地址开始查找，而是从上次找到的空闲区的下一个空闲区开始查找。

    - 最佳适应算法 (Best Fit): 空闲分区按容量递增次序排列，每次分配时，顺序查找到第一个满足要求的空闲区进行分配。

    - 最坏适应算法 (Worst Fit): 空闲分区按容量递减次序排列，每次分配时，顺序查找到第一个满足要求的空闲区进行分配。

- **基于索引搜索的分配算法**:

    - 快速适应算法 (Quick Fit): 将空闲分区按大小分类，每个类形成一个空闲链表，分配时，找到第一个满足要求的空闲区进行分配。

    - 伙伴系统 (Buddy System): 将空闲分区按2的幂次方大小分类，每个类形成一个空闲链表，分配时，找到第一个满足要求的空闲区进行分配。

    - 哈希表 (Hash Table): 将空闲分区按哈希值分类，每个类形成一个空闲链表，分配时，找到第一个满足要求的空闲区进行分配。
