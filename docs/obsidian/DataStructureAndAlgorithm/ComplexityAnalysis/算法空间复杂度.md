...
![算法相关空间](assets/space-complexity/space_types.png)
...
!!! tip
    假设有两个实现相同功能的程序，一个使用[**递归**](迭代与递归.md#递归)，一个使用[**迭代**](迭代与递归.md#迭代)，且时间复杂度相同；则递归版本的空间复杂度（$\geqslant O(\log n)$，有尾递归优化时可达 $O(1)$）在大多数情况下要比迭代的空间复杂度（基本为 $O(1)$）高阶。

    以[迭代与递归](迭代与递归.md)中贯穿大半个文章的求和程序为例，无论是迭代还是线性/尾递归还是迭代的版本，几者的时间复杂度都是 $O(n)$；
