...
!!! review
    这个bug的核心原因在于 C 局部变量**不会自动清零**, [C Basics](../编程语言/c/C%20Basics.md#Variable-declaration-and-initialization).
...
<!-- 若用 `scanf("%s", input)` 往只有 5 个字节的数组里读 `"12345"`: `'\0'` 写出界, 正好踩到相邻的循环变量 `i`. 用 `watch input[5]` 会发现每次回到 `for` 开头 `i++` 时, 这块内存也在变, 于是确认两块存储是挨着的. 数组越界和指针见 [C Pointers, Arrays, and Strings](../编程语言/c/C-Pointers-Arrays-and-Strings.md).

`x` (examine) 按原始字节看内存, 不依赖变量类型:
...
!!! warning "`scanf` 读字符串"
    `scanf("%s", buf)` 不检查长度, 超长输入会破坏栈上相邻变量, 严重时在函数返回才崩. 调试练习可以用来理解越界, 正式代码应限制宽度 (`%4s`) 或改用更安全的读法.

## 段错误

程序收到 `SIGSEGV` 时, 在 gdb 里 `run` 会自动停在出事地点, 再用 `bt` 看是哪一层调用进来的.

典型例子: `scanf("%d", man)` 少写了 `&`, 把 `man` 的值当成地址去写, 崩溃点往往在 libc 的 `_IO_vfscanf` 里. `bt` 会显示真正的调用点在 `main` 的那一行 `scanf`.

另一类更绕: 局部数组越界当时不一定立刻 SIGSEGV, 而是把返回地址或栈上别的控制数据踩坏, **函数返回时** (对应源码里那个 `}`) 才崩. gdb 指到函数末尾的 `}` 时, 优先怀疑本函数里的缓冲区和指针, 而不是去盯那一行空括号.
...
[Pwndbg](https://pwndbg.re/stable/) (`/paʊnˈdiˌbʌɡ/`) 是 GDB / LLDB 的插件, 每次停下自动刷新一块 **context**: 寄存器, 反汇编, 栈, 回溯, 以及指针的多层解引用. 普通 gdb 里要反复敲 `i r`, `x/32gx $rsp`, `bt`; 有了 context, 这些信息默认就在眼前.
