...
Git的设计原则是**只跟踪文件**，而不直接跟踪空目录。因此在一个目录为空时，Git在commit或push时repo是不会包含这个目录的。

在项目需要预留目录用于缓存、日志时，就可以考虑使用一个**占位文件**来使得这个目录随着这个占位文件被跟踪。文件内容通常为空，也可以添加注释说明用途。

需要注意的是，gitkeep并不是官方属性，只是Git属性的一个简单应用。这个占位文件名可以是任意的，但是根据社区约定，一般采用`.gitkeep`。
