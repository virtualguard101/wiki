...
**Socket Interface（套接字接口）** 是[操作系统提供的一套标准 API](../OperatingSystem/408/进程的描述与控制.md#套接字)，用于在进程之间进行网络通信，也可以用于本地 IPC（Inter-Process Communication, [进程间通信](../OperatingSystem/408/进程的描述与控制.md#进程通信)）。它最早由 BSD Unix 提出，后来成为几乎所有操作系统（Linux、Windows、macOS 等）的网络编程事实标准。
...
[TCP连接](408/TCP.md#TCP连接管理)采用[客户端/服务端模式](../OperatingSystem/408/进程的描述与控制.md#客户端-服务器系统)，按两个不同的角色进行分类，其主要系统调用如下：
