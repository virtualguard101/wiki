...
如果对于随机变量 $X$ 的[分布函数](随机变量的分布函数.md) $F(x)$，存在非负函数 $f(x)$，使得对于任意实数 $x$，有：
...
方差和方差的计算公式与[离散型随机变量](离散型随机变量及其分布.md)相同：
...
!!! info "指数分布的数学期望与方差"
    ...
    &= \int_{0}^\infty x^2 \lambda e^{-\lambda x} dx - \left(\frac{1}{\lambda}\right)^2 \\
