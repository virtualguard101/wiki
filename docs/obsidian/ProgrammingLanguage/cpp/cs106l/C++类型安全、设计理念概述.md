...
1. 只有在解决实际问题时才添加新特性
    ...
    - [`std::unique_ptr`](RAII%20&%20C++智能指针.md#智能指针) 和 [`std::shared_ptr`](RAII%20&%20C++智能指针.md#智能指针) 解决了动态内存管理的问题

    - [Lambda 表达式](Lambda表达式.md)解决了创建小型匿名函数的问题
...
4. 允许程序员在需要时完全控制
    ...
    - 可以使用高级抽象（如[标准库容器](STL容器.md)、算法）

    - 也可以直接操作内存（通过[指针](迭代器和指针.md#指针)、[引用](初始化与引用操作.md#引用)）
...
5. 除非万不得已，否则不牺牲性能
    ...
    - [RAII](RAII%20&%20C++智能指针.md)（资源获取即初始化）模式提供安全的资源管理，无需额外的运行时检查

    - [移动语义](C++移动语义.md)减少了不必要的复制操作

    - 编译时多态（通过[模板](C++类%20&%20模板.md)）避免了运行时虚函数调用的开销

6. 尽可能在编译时强制安全性

    C++ 通过类型系统和编译时检查来捕获错误：

    - [强类型系统](基础类型与结构体.md#基础数据类型)防止类型错误

    - [`const`](初始化与引用操作.md#const-关键字) 和 [`constexpr`](C++类%20&%20模板.md#constexpr) 确保不可变性

    - [模板元编程](C++类%20&%20模板.md#模板元编程)允许编译时计算和验证
