...
JavaScript 代码基本与其他主流编程语言（如 [C++](../cpp/cs106l/基础类型与结构体.md)、[Java](../java/Java入门要点.md)等）相通，代码块以大括号`{}`进行分隔
...
对象解构是一种简化操作，形式上有些类似于C++中的[结构化绑定](../cpp/cs106l/初始化与引用操作.md#结构化绑定)，相比较上面的访问方式，可一次性获取对象的多个属性：
```js
const person = {
    name: "virtualguard",
    id: 101
