...
- 使用 `:=` 关键字声明变量（初始化声明）
    ...
    这种声明方式类似C++中的[auto关键字](../cpp/cs106l/基础类型与结构体.md#auto-关键字)，是声明变量的首选方式，但只能用于**局部变量**的声明。
...
常量声明可以以**编译期**中可以确定结果的表达式作为[右值](../cpp/cs106l/初始化与引用操作.md#左值与右值):
...
与C++中的[结构体](../cpp/cs106l/基础类型与结构体.md#结构体struct)类似，Golang中的结构体也是一种复合数据类型，用于表示一组相关数据的集合。
