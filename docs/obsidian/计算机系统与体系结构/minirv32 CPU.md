...
- `jalr`指令还需要**写入PC寄存器**, **从PC寄存器读取地址**并**写回**通用寄存器. I 型四条指令的写回是两级选择; `lbu` 只改 load 那一路的数据, `addi` / `jalr` 不用动:
    ...
    其中 $\langle addr[1:0]\rangle$ 表示按[字节槽](#RAM)选出的那 8 位. 电路上: `lbu` 去选 "零扩展字节 / 整字", 再把结果接到现在 `LS` 那个 mux 的 RAM 数据端.
...
- I 型的 `RAMren` 与这里的字节使能可以在 main 里汇总: 读不写时使能为 0, 写时 `RAMren` 可忽略 (异步读口仍然能给出当前字, 只是 `lbu`/`lw` 才用它).
...
地址也不必仲裁: `rd` / `rs1` / `rs2` 在指令里的位段与格式无关, 四路译码吐出的地址始终是同一组数. 交互模块只剩两件事: **要不要写**, **写什么数据**. S 型不写 GPR, 也不再需要 `Sramwe` 去选读地址.

#### 接口规划

交互模块只接写通道. 地址在 main 里从指令分线直出, 不再作为本模块端口:

| 来源 | 使能 | 写数据 |
|:-----|:-----|:-------|
| `Inst_I_dec` | `Iwe` | `Iwdata` |
| `Inst_R_dec` | `Rwe` | `Rwdata` |
| `Inst_U_dec` | `Uwe` | `Uwdata` |
...
\textit{GPRwdata}  &= (I_{wdata} \land I_{we})
                    \lor (R_{wdata} \land R_{we})
                    \lor (U_{wdata} \land U_{we}).
\end{aligned}
$$

写数据用独热门控, 不要优先编码器, 也不要带 enable 的 mux:

- `Iwe` / `Rwe` / `Uwe` 各零扩展到 32 位, 与对应 `wdata` 按位与; 未命中的那路变成 0.

- 三路再按位或. 独热保证最多一路非 0; 全 0 时 `GPRwdata = 0`, `GPRwe = 0`, 不会写入, 也不会出现高阻.

Logisim 里就是: 3 个 32 位与门 + 1 个三输入或门 + 1 个 1 位或门. 组合深度两级, 没有选择端极性问题.

`lui` 不读寄存器, 但读地址仍然接 `inst[18:15]` / `inst[23:20]`. 读到的数不用即可, 比把地址置 Z 更干净.
