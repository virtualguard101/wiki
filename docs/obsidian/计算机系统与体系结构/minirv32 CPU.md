...
!!! warning "字节地址与字地址问题的实例"
    ...
    初学者在看到文档写 `PC=0x4`, 可能会产生误区, 认为应该在 ROM 的下标 `4` 填指令, 并在 `1..3` 塞 `0`. 那样只有 `PC=0` 时能取到第一条 `addi`; 执行到 `PC=4` 时取的是 `ROM[1]=0`, 后面的跳转目标也会全部错位, 仿真结果会和预期完全对不上.

    调试时若盯着 ROM 面板, 会看到字地址 `0x1` / `0x5`; 文档对照请用字节 PC `0x4` / `0x14`. 两者相差的就是那个 `<< 2`, 不是程序本身装错了.
...
!!! failure
    ...
    除此之外还要落实 [GPR](#GPR) / [零寄存器 `x0`](risc-v/RV32I%20Reference%20Card.md#The-Zero-Register) 的约定: **读恒为 0, 写忽略**. 若把 `x0` 做成普通寄存器, 即使跳转已经接对, 仍可能出现「`a0` 对了却进不了自旋」:

    - `0x14` 的 `jalr zero, 0(ra)` 会把 `PC+4` 写进 `x0`
    - 随后 `0x8` 的 `jalr ra, 12(zero)` 用的基址已不是 0, 目标变成 `x0+12`, **到不了** `0xc`
    - 即便到了 `0xc`, `jalr zero, 12(zero)` 也会污染 `x0`, 下一拍跳出自旋

    电路上至少: `waddr==0` 时关掉写使能; 读口 `raddr==0` 时输出常数 `0`:

    ![](assets/minirv32-cpu/7.png)

!!! important "调试的一般步骤"
    以修复第一版 `jalr` 时的几轮仿真为例. 建议每次只改一类问题, 并用**字节 PC** (不是 ROM 字地址) 对照文档轨迹: `0 → 4 → 10 → 14 → 8 → c → c → ...`.

    **1. 仿真先看控制流, 再看寄存器末态**

    第一版修复里曾把 PC 的 MUX 误接到 `+4` **之前**, 变成 `PC ← MUX(PC, 目标) + 4`, 电路类似:

    ![](assets/minirv32-cpu/6.png)

    现象: ROM 面板上在字地址 `0x1` 与 `0x5` 之间横跳 (对应字节 PC `0x4` ↔ `0x14`). 调用本该进 `0x10`, 实际落到 `0x14`; 若此时链接又没写对, 返回会立刻跳回调用点. **MUX 应接在 PC 的 `D` 端**, 两路是 `PC+4` 与 `(rs1+imm)&~1`, 不要对跳转目标再 `+4`.

    **2. 核对写回数据是不是 `PC+4`**

    写回 MUX 若在 `jalr` 时误接 `PC` (或误接「即将写入 PC 的跳转目标」), 调用处会把 `ra←4` 而不是 `ra←8`, 返回后在 `0x4 → 0x10 → 0x14 → 0x4` 打转, `a0` 还会反复 `+10`. `jalr` 的 `wdata` 必须是 **当前指令的 `PC+4`**.

    **3. 核对 ROM 内容与编码**

    - 按[字节 / 字地址](#字节地址与字地址问题的实例)密集装填, 不要按字节 PC 在 ROM 里留空.
    - 手工填十六进制时对照 I 型字段: `imm` 在 `inst[31:20]`, 不要把 `0x010000e7` 写成 `0x000100e7` (会变成 `jalr ra, 0(x2)`, 直接 `0 ↔ 4` 死循环).

    **4. 末态 `a0=30` 仍要确认是否停在 `0xc`**

    寄存器对只说明算术 / 调用主路径大致对; **停机**取决于 `PC` 是否稳定在 `0xc` (ROM 字地址 `0x3`), 以及 `x0` 是否始终为 0. 两者都满足, 才算这段测试程序通过.
