...
##### 寄存器交互
    ...
    - `jalr`指令还需要**写入PC寄存器**, **从PC寄存器读取地址**并**写回**通用寄存器. I 型四条指令的写回数据不是二分, 而是两级选择 (load / jalr / addi):
        
        $$
        \textit{GPRwdata} =
        \begin{cases}
        M[R[rs1]+\textit{imm}] & lbu \lor lw \\
        PC+4                   & jalr \\
        ...
        `lbu` 在写入 GPR 前还要从字里按 `addr[1:0]` 抽出对应字节并零扩展, 不能直接用整字 `M[]`.
...
- 加载-存储操作
    ...
    - 涉及RAM的地址解析同[I型指令的两个加载指令](#I型指令汇总解码).
...
针对指令的解码模块按照指令格式划分成了四种, 上面以这四种指令格式为依据实现了四种指令的解码电路, 这些电路中均存在GPR交互的需求, 因此可以考虑将这些GPR交互的需求抽象出来, 单独实现一个GPR交互模块.

输出最为简单, 只需要与 GPRs 的输入对齐即可; 输入也并不难处理, 将每个译码模块的输出进行标注并汇总即可.

关键在于应该如何控制、选择执行各条指令时真正接到 GPRs 上的那一组交互信号.
...
读数据是 GPRs 的**输出**, 直接扇出回各译码模块的 `rs1data` / `rs2data` 即可, 不必再进交互模块. 交互模块只负责把四路译码结果**仲裁成一组**接到 GPRs 的输入口.
...
同一时刻最多只有一种格式命中(`opcode`, `funct3`组合互斥), 因此这四路使能`{Iwe, Rwe, Uwe, S}`是**独热**的. 由于译码模块的划分基于指令格式, 交互模块并不需要"认出 8 条指令", 仅需用这4个独热位选出当前格式对应的那一组地址或数据.

#### 接口规划

| 来源 | 使能 | 写地址 | 读口 0 | 读口 1 | 写数据 |
|:-----|:-----|:-------|:-------|:-------|:-------|
| `Inst_I_dec` | `Iwe` | `Iwaddr` | `Iraddr0` | — | `Iwdata` |
| `Inst_R_dec` | `Rwe` | `Rwaddr` | `Rraddr0` | `Rraddr1` | `Rwdata` |
| `Inst_U_dec` | `Uwe` | `Uwaddr` | — | — | `Uwdata` |
| `Inst_S_dec` | `Sramwe` (即 `sb ∨ sw`, 只用来标明"当前是 S 型") | — | `Sraddr0` | `Sraddr1` | — |

S 型没有 `we` / `waddr` / `wdata`, 因为其没有写需求; 用 `Sramwe` 充当"S 型有效"只是为了给读地址 mux 一个选择信号.
...
\textit{GPRwe}
  &= I_{we} + R_{we} + U_{we}, \\
\textit{sel}_w
  &= \mathrm{enc}(I_{we}, R_{we}, U_{we}), \\
\textit{GPRwaddr}
  &= \mathrm{mux}(\textit{sel}_w,\; I_{waddr},\; R_{waddr},\; U_{waddr}), \\
\textit{GPRwdata}
  &= \mathrm{mux}(\textit{sel}_w,\; I_{wdata},\; R_{wdata},\; U_{wdata}).
\end{aligned}
$$

对读口 0, 三路争用 (`I` / `R` / `S`); 对读口 1, 两路争用 (`R` / `S`):

$$
\begin{aligned}
\textit{GPRraddr0}
  &= \mathrm{mux}\big(\mathrm{enc}(I_{we}, R_{we}, S),\; I_{r0},\; R_{r0},\; S_{r0}\big), \\
\textit{GPRraddr1}
  &= \mathrm{mux}(R_{we},\; S_{r1},\; R_{r1}).
\end{aligned}
$$

电路上 `enc` 用优先编码器把独热使能编成 mux 的二进制选择端; mux 的使能接到对应几路 we 的或, 未命中时输出*高阻*, 此时 `GPRwe = 0`, GPRs 不会写入, 高阻写数据可以忽略.

`lui` 不读寄存器, 读地址 mux 会关掉, `raddr` 呈高阻. 仿真里 GPRs 的 `Q0`/`Q1` 可能显示为红色(表示数据错误), 但不影响 `lui` 的写回.

!!! success "简化"
    I / R / U 的 `waddr` 都来自 `inst[11:7]` 的低 4 位, I / R / S 的 `raddr0` 都来自 `inst[19:15]` 的低 4 位, R / S 的 `raddr1` 都来自 `inst[24:20]` 的低 4 位. **这些字段在指令编码里的位置与格式无关**, 所以各译码模块输出的地址其实始终是同一组数.

    因此地址 mux 在功能上是冗余的, 只留下 `GPRwe` 和 `GPRwdata` 真正需要仲裁, 可以简化:
    ...
    \textit{GPRwdata}  &= I_{we}\,?\, I_{wdata} : (R_{we}\,?\, R_{wdata} : U_{wdata}).
    \end{aligned}
    $$

    上面的实现方案仍按照每路译码各给一套地址, 再使用复用器来接; 后面若改, 优先把地址改成直接分线.

    读数据路径则更简单: `GPRs.Q0` / `Q1` 接到 I / R / S 译码器的 `rs1data` / `rs2data`. 写发生在时钟沿, 读是组合的, 本拍写入的值下一拍才能读到, 单周期 CPU 正需要这样.

### 当前设计里已经能看出来的问题

对照文档约定和 `csa/minirv32.circ` 的现有接线, 下面这些问题不修的话, 后面用 `mem.hex` 那种大量 `lbu`/`sb` 的程序会直接跑飞.

#### 2. 字节访存几乎还没做

文档要求 RAM "使用字节启用", `lbu` 要按地址取出**一个字节再零扩展**, `sb` 只改目标字里对应的那一个字节. 现状是:

- RAM 仍是 32 位端口, **没有**字节使能, 还开了清除销 (文档写的是不用).
- `Inst_I_dec` 在 `lbu`/`lw` 时把整字 `RAMdata` 直接当作 `GPRwdata`, 两条 load 没有区别.
- `Inst_S_dec` 的 `RAMwdata` 直接等于 `R[rs2]` 的 32 位, `sb`/`sw` 也没有区别.

`mem.asm` 里大量 `lbu gp, 1(a4)` 这种非对齐字节访问, 现在读到的会是整个字, 字符串、栈上的单字节都会错.

#### 3. RAM 的地址抽象和 ROM 不一致

取指已经按 `PC[17:2]` 做字地址. 数据 RAM 同样是 32 位数据口、16 位地址口, 但接到 RAM 的是字节地址的低 16 位 `addr[15:0]`, 没有 `>> 2`.

于是 `lw`/`sw` 的 `addr = 0, 4, 8` 会打到 RAM 字 `0, 4, 8`, 中间空三格; 和汇编/链接脚本里的字节地址对不上, 也和 ROM 的映射规则分裂成两套.

#### 4. `jalr` 仍缺 `& ~1`

`Inst_I_dec` 的 `PCdata` 在 `jalr` 时直接送 `R[rs1]+imm`, 没有把 bit 0 清掉. 取指若只用 `PC[17:2]`, 奇数 PC 碰巧还能取到同一条指令, 但链接寄存器里的 `PC+4` 会变成奇数, 和 RV32I 语义不一致.

#### 5. I 型写回 mux 结构对, 但 `lbu` 仍缺字节提取

`Inst_I_dec` 里已经是两级 mux: 先按 `jalr` 在 `ALU` 与 `PC+4` 之间选, 再按 `LS = lbu \lor lw` 插入 `RAMdata`. 这和上面的 `GPRwdata` 方程一致; 缺的是 `lbu` 对 `RAMdata` 的字节选择与零扩展.

#### 6. 立即数扩展器: 默认多半是对的, 但仍应点开确认

I 型和 S 型的 Bit Extender 在 `.circ` 里没有写出 `type`. Logisim-evolution 新建时默认是 **sign**, 这正好符合 I/S 立即数的 ISA 要求. 仍建议在元件属性里确认显示的是 Sign 而不是 Zero; 负立即数 (`addi a0, zero, -6`, `lbu gp, -16(sp)`) 是最直接的回归.

#### 7. 寄存器号被截成 4 位

各译码器把 5 位 `rd`/`rs1`/`rs2` 丢掉最高位再送给 16 个 GPR. 这与 RV32E 的约定一致, 但若误用 RV32I 工具链编出 `x16`–`x31`, 会别名到 `x0`–`x15`; 写 `x16` 等于写 `x0`, 而 `x0` 硬接线为 0, 看起来会像"指令没写进去".

#### 8. GPR 交互模块本身的隐患

- 优先编码器接了 4 个输入, 第 4 路恒 0, 对应 4:1 mux 的第 4 路悬空. 独热正常时选不到, 但编码器和 mux 的**输入编号是否都从上往下为 0**必须和仿真一致, 否则 `addi` 的结果会写进 `add`/`lui` 的通道.
- `GPRraddr1` 的 2:1 mux 用 `Rwe` 做选择. 若极性接反, 本应选 `Rraddr1` 时选到 `Sraddr1`. 目前两者都是 `inst[23:20]`, **错误被相同的字段值掩盖了**, 地址改成"直接分线"之前不会爆, 一改就会爆.
- 未命中时读地址为高阻, `lui` 或非法 opcode 时 GPRs 读口可能在仿真里报错.

建议验证顺序: 先修 `sw` 的 `funct3` 并确认符号扩展 → 用负立即数的 `addi` 和 `jalr` 自旋环 → 再给 RAM 补字节使能 / 字地址截取 → 最后才值得用完整 `mem.hex` 做冒烟.
