...
![链表](assets/linked/linkedlist_definition.png)
...
!!! tip
    链表还可以以一种递归的形式创建与解析，详情可参考[递归对象-链表 | COMPOSING PROGRAMS 中译版](https://composingprograms.netlify.app/2/9#_2-9-1-%E7%B1%BB-%E9%93%BE%E8%A1%A8)。
...
时间复杂度为 $O(n)$。链表没有随机访问, 不能改用[折半查找](顺序查找和折半查找.md#折半查找), 这就是链表上的[顺序查找](顺序查找和折半查找.md#一般线性表的顺序查找).
...
**双向链表**的本质是在[单链表](#单链表)的每个节点中再设置一个指向其**前驱节点**的指针域。
