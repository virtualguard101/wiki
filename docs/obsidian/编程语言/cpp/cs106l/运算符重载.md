...
`friend`（友元）用于在**访问控制**上开例外：被声明为友元的函数或类，可以访问另一个类的 **private / protected** 成员，就像被授权进入「内部」的外部代码。

### 默认规则

- 类外代码**不能**直接访问别的类的 private 成员。
- **成员函数**运算符（如 `bool operator<(const T& rhs) const`）在类内，可直接用 `this` 和成员。
- **非成员函数**运算符写在类外，两侧操作数都是参数；若需要读 private 成员，就要么把成员设为 public，要么使用 `friend`。

### 友元函数

**友元函数**不是类的成员函数，但被允许访问该类的 private / protected 成员。常见用途：

- 运算符重载（尤其是需要对称、非成员的 `==`、`<<` 等）；

- 紧密协作的另一函数/类；

- 某些测试辅助（也可用，但更常见是测 public 接口）。

### 与运算符重载：`operator==` 示例

下面写法把 **非成员** `operator==` 声明为友元，因此可以访问 `octets`（若将 `octets` 改为 `private` 仍然合法）：
...
| 写法 | 特点 |
|------|------|
| 成员 `operator==` | 左侧一定是本类对象：`a == b` 等价于 `a.operator==(b)` |
| 友元 `operator==` | 对称的两个参数；便于以后扩展（如与别的类型比较） |

若成员已是 public，技术上**不必**写 `friend` 也能在类外实现 `operator==`；仍用 `friend` 的意义是：以后把数据成员收成 private 时，比较逻辑不必挪走，封装边界更清晰。

### 友元的其他形式

1. **友元函数**：在类内声明（可内联定义），或类内声明、类外定义。

2. **友元类**：整个类都能访问你的 private（例如迭代器访问容器内部）。

3. **友元成员函数**：另一个类的**某个**成员函数是友元。
...
class Foo {
  int x_{};
  friend void Dump(const Foo& f);
};

void Dump(const Foo& f) {
  std::cout << f.x_;  // 友元可访问 private
}
```

### 重要性质

- **不传递**：A 是 B 的友元，B 是 C 的友元，**不代表** A 能访问 C 的 private。

- **不继承**：派生类**不会自动**继承基类授予的友元权限。

- **单向**：「X 是 Y 的友元」只表示 X 能看 Y，**不是** Y 能看 X。

- **慎用**：友元过多会破坏封装、增加耦合；适合少量、理由明确的访问（运算符、`<<`、紧密协作类型）。

### 与非成员重载的配合

非成员运算符若需访问 private，典型模式是：
...
- `operator<<`：左侧是 `ostream`，**不能**写成 `Vector3` 的成员，常用友元非成员函数。

- `operator*(float, Vector3)`：左操作数是 `float`，也**不能**写成只有 `Vector3` 参与的成员形式，故用友元实现 `scalar * vec`，内部再委托 `vec * scalar`。

下文 [实际应用](#实际应用) 中的 `Vector3` 即采用这两种友元。
