...
了解过[C语言](../../c/C-Pointers-Arrays-and-Strings.md#Pointers-Store-Addresses)的话，这部分内容可直接跳过。是的，二者的指针没有本质上的区别。

详细概念、底层原理可参考[Pointers and Memory | CS106L-TextBook](https://cs106l.github.io/textbook/cpp-fundamentals/pointers-and-memory)

### 指针与引用的区别

[引用](初始化与引用操作.md#引用)本质上是一个变量的**别名**，而指针的本质是其指向变量的**内存地址**。访问引用变量的值只需要使用引用的名称即可，而访问一个指针指向的变量的值则需进行**解引用**操作（`*`）。
