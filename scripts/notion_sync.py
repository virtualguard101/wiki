...
# Match fenced code so converters do not rewrite shell/CMake/$var examples inside them.
_FENCE_RE = re.compile(r"```[\s\S]*?```|~~~[\s\S]*?~~~")


def map_outside_fences(text: str, transform: Any) -> str:
    """Run ``transform`` on markdown outside fenced code blocks only."""
    slots: List[str] = []

    def stash(match: re.Match[str]) -> str:
        slots.append(match.group(0))
        return f"⟦CODEFENCE:{len(slots) - 1}⟧"

    masked = _FENCE_RE.sub(stash, text)
    transformed = transform(masked)

    def restore(match: re.Match[str]) -> str:
        return slots[int(match.group(1))]

    return re.sub(r"⟦CODEFENCE:(\d+)⟧", restore, transformed)
...
def convert_inline_math(text: str) -> str:
    ...
    # $0 / $uri / ${PROJECT_NAME} inside ``` fences must stay literal.
    return map_outside_fences(text, transform)
