...
class FenceProtectionTests(unittest.TestCase):
    ...
    def test_shell_scripting_note_fences_untouched(self) -> None:
        note = Path(__file__).resolve().parents[1] / "docs/obsidian/Tools/Shell Scripting.md"
        ...
        self.assertIn('echo "用户: $username, UID: $uid, 主目录: $homedir"', out)
        self.assertIn('echo "$HTML_FILES" | grep "/index.html$"', out)
        self.assertNotIn("$`0 has called`", out)
        self.assertNotIn("$`username, UID:`", out)
        self.assertNotIn('$`HTML_FILES" | grep "/index.html`', out)
